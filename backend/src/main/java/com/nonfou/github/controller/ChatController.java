...
public class ChatController {
    ...
    public Object claudeMessages(
        ...
        log.info("Claude API 兼容接口调用: /v1/messages, model={}, anthropic-beta={}, anthropic-version={}",
                request.getModel(), anthropicBeta, anthropicVersion);

        // Claude API 使用 x-api-key header，需要转换为 Authorization
        String authHeader = xApiKey != null ? "Bearer " + xApiKey : authorization;
        // 保存 anthropic-beta 和 anthropic-version 头到请求中
        request.setAnthropicBeta(anthropicBeta);
        request.setAnthropicVersion(anthropicVersion);
    ...
    private Object handleClaudeRequest(String authorization, ClaudeRequest request) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ClaudeErrorResponse.fromStatusCode(500, e.getMessage()));
        } catch (Exception e) {
            log.error("Claude API调用失败: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ClaudeErrorResponse.fromStatusCode(500, "服务暂时不可用，请稍后重试"));
        }
    }
     ...
     * Claude 格式流式聊天接口
     */
    private Object claudeStream(String authorization, ClaudeRequest request) {
        try {
